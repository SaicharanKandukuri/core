...
from .const import CONF_INDEX, CONF_SWITCH, DOMAIN
    ...
    return vol.Schema(
        {
            vol.Required(CONF_INDEX): cv.multi_select(
                {
                    str(index): config[CONF_SWITCH]
                    for index, config in enumerate(handler.options[SWITCH_DOMAIN])
                },
            )
        }
    )
...
async def validate_sensor_setup(
    ...
    sensors: list[dict[str, Any]] = handler.options.setdefault(SWITCH_DOMAIN, [])
...
async def validate_import_sensor_setup(
    ...
    for process in import_processes:
        sensors.append({CONF_SWITCH: process})
...
async def validate_remove_sensor(
    ...
    sensor: dict[str, Any]
    for index, sensor in enumerate(handler.options[SWITCH_DOMAIN]):
        if str(index) not in removed_indexes:
            sensors.append(sensor)
        elif entity_id := entity_registry.async_get_entity_id(
            SWITCH_DOMAIN, DOMAIN, slugify(f"process_{sensor[CONF_SWITCH]}")
        ):
            entity_registry.async_remove(entity_id)
    handler.options[SWITCH_DOMAIN] = sensors
    return {}


SENSOR_SETUP = vol.Schema(
    {
        vol.Required(CONF_SWITCH): TextSelector(),
    }
)
...
OPTIONS_FLOW = {
    "init": SchemaFlowMenuStep(["add_switch", "remove_switch"]),
